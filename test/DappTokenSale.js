...
contract('DappTokenSale', function(accounts) {
	...
	return DappToken.deployed().then(function(instance){
		...
		// Try to buy tokens differnt from the eth value
		...
		return tokenSaleInstance.buyTokens(800000, {from: buyer, value: numberOfTokens * tokenPrice });
	 	...
	 	assert(error.message.toString().indexOf('revert') >= 0, 'Cannot purchase more tokens then available');
});
});
 ...
 it('ends token sale', function(){
		...
		// then grab token sale instance
		tokenSaleInstance = instance;
		// Try to end sale by account other then the admin
		return tokenSaleInstance.endSale({from: buyer});
 			...
 			assert.equal(balance.toNumber(), 999990, 'returns all unsold tokens to the owner/admin');
